...
<head>
        ...
        <link rel="stylesheet" href="../css/style-envio-realizado.css">
    </head>
    <body>
<!-- navbar -->
<nav class="navbar navbar-expand-lg bg-light sticky-top">
    ...
    session_start();
    $id_costo = $_SESSION['id_costo'];

    $query = "INSERT INTO tracking (no_orden,estado, direccion, destinatario, id_tienda, id_costo) VALUES (null, '1','$direccion', '$nombre', 1, $id_costo)";
    $result = mysqli_query($con,$query) or die('Query failed: ' . mysqli_error($con));

    if($result) {
        ?>
        <div class="aviso-str">
        <h3>ENVIO REALIZADO CON EXITO!</h3>
    </div>
        <?php
    }else{
        echo "<p>Lo sentimos no tenemos envios disponibles para la ubicacion del destino</p>";
    }
    $queryguia = "SELECT * FROM tracking WHERE direccion='$direccion' AND destinatario='$nombre' AND id_costo='$id_costo'";
    $resultguia = mysqli_query($con,$queryguia) or die('Query failed: ' . mysqli_error($con));

    if($resultguia){
        while( $row = mysqli_fetch_array($resultguia)) {
            $guia = $row["no_guia"];
        }
        ?>
        <div class="aviso-guia">
        <h3>Su numero de guia es: <?php echo $guia;?></h3>
        </div>
        <div class="boton">
            <a href="../index.html" class="btn btn-outline-primary" name="submit" type="submit">REGRESAR</a>
        </div>
        <?php
    }
    mysqli_close($con);
